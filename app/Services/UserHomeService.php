...
class UserHomeService
{
    public function getHomeData(): array
    {
        ...
        $products_offer = Product::withAvg('ratings', 'rate')->withAvg('pharmacistRatings', 'rate')
            ->with('offer')
            ->whereHas('offer')
            ->where('active', true)
        ...
        $top_rate_products = Product::withAvg('ratings', 'rate')->withAvg('pharmacistRatings', 'rate')
            ->with('offer')
            ->where('active', true)
        ...
        $to_sell_products = Product::withSum('orderItems', 'quantity')->withAvg('pharmacistRatings', 'rate')
            ->where('active', true)
            ->orderByDesc('order_items_sum_quantity')
            ->take(10)
            ->get();

        $features = Product::where('is_featured', true)->withAvg('pharmacistRatings', 'rate')
            ->where('active', true)
            ->with('offer')
            ->latest()
            ->take(10)
